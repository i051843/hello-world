...
def pomMap
def credId
def ose3Project = "stratus"
def ose3TokenID = "OPENSHIFT_DEV_TOKEN_CREDENTIAL_ID"
def ose3Region = "https://api.boaw.paas.gsnetcloud.corp:8443"
    ...
    stages {
                ...
                script {
                    if (BRANCH_NAME != MASTER_BRANCH) {
                        ...
                        if (result == 0) {
                            ...
                            error('Stopping due to ci-skip comment')
                        }
                    }
                }
                script {
        ...
        stage('Build') {
            ...
            steps {
                ...
                script {
                    // reads the maven pom
                    if (params.IS_RELEASE) { //TODO change
            ...
            steps {
                script {
                    if (params.DEPLOY_ONLY) {
                        artifactUrl = almMavenNexusResolver(
                                groupId: "${pomMap['POM_GROUPID']}",
                                artifactId: "${pomMap['POM_ARTIFACTID']}",
                ...
                script {
                    almOpenshiftDeploy(
                            ...
                            ose3Template: 'javase',
                            ose3TokenCredentialId: "${ose3TokenID}",
                            ose3TemplateParams: [
                                    ARTIFACT_URL: artifactUrl,
                                    JAVA_OPTS_EXT: "-Dfile.encoding=UTF-8"
                            ]
                    )
                }
            }
